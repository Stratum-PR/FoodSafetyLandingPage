...
(function(){
const TEST_MODE = false;     // true = log submissions to the console instead of sending them
const REQUIRE   = false;     // answers optional while validating the flow
const SUBMIT_URL = "/api/selfcheck"; // Vercel function: emails the passport + notifies contact@stratumpr.com
...
const T={
 es:{
  ...
  updatesOptIn:"Quiero recibir noticias de Stratum (opcional). Te pediremos confirmarlo por correo.",
  mailSending:"Enviando tu pasaporte a", mailSent:"Te enviamos una copia de tu pasaporte a", mailFail:"No pudimos enviarte el correo. Puedes guardarlo en PDF con el botón de arriba.",
 ...
 en:{
  ...
  updatesOptIn:"I want to receive news from Stratum (optional). We'll ask you to confirm by email.",
  mailSending:"Sending your passport to", mailSent:"We emailed a copy of your passport to", mailFail:"We couldn't send the email. You can save it as a PDF with the button above.",
...
T.es.tiers=window.StratumSelfcheck.TIERS.es; T.en.tiers=window.StratumSelfcheck.TIERS.en;
T.es.secNames=window.StratumSelfcheck.SECTIONS.es; T.en.secNames=window.StratumSelfcheck.SECTIONS.en;

/* ---------------- scored items (the 16 requirements) ---------------- */
const {ITEMS}=window.StratumSelfcheck;
...
/* ---------------- state ---------------- */
const S={lang:"es",screen:"step",step:0,c:{},v:{},m:{},lab:false,anim:true,gift:false,calc:["","","","","30"],simQ:"",simOpen:false};
...
function renderContact(){
  ...
  app.innerHTML=`<div class="screen ${S.anim?'enter':''}">${progress(true)}
   ...
   <label class="optin"><input type="checkbox" id="f-updates" ${c.updates?"checked":""}><span>${esc(t("updatesOptIn"))}</span></label>
...
function renderStep(){
  ...
  primary.textContent=t("next"); // the last step leads to the contact screen, not straight to results
  backBtn.classList.toggle("show", _l.indexOf(S.step)>0); // nothing before step 1
}
function renderResults(){
  ...
  const showCalc=S.v.price!=null && S.v.price!==0;   // any answer to the price question except "Sí, tengo claro…"
  ...
  app.innerHTML=`<div class="screen ${S.anim?'enter':''}">
   ...
   ${S.mail?`<p class="mail-note is-${S.mail}" id="mail-note" role="status">${mailNote()}</p>`:""}
   </aside>
...
function saveContact(){
  ...
  if(S.screen==="contact") S.c={name:g("f-name"),biz:g("f-biz"),email:g("f-email"),phone:g("f-phone"),updates:!!document.getElementById("f-updates")?.checked};
}
...
app.addEventListener("click",e=>{
  ...
  if(a.dataset.act==="restart"){ S.v={}; S.m={}; S.lab=false; S.mail=null; to("step",0); }
});
...
/* ---------------- submission ---------------- */
/* ---------------- submission ----------------
   "completed" emails the passport and notifies Stratum; "lab_connect" and
   "design_partner" are hot leads. Other events stay in the browser for now. */
const SENT_STAGES=["completed","lab_connect","design_partner"];
function report(){
  const r=score(), ks=activeItems();
  return {points:r.pts,max:r.max,pct:Math.round(r.pct),blockers:r.blk,tier:r.tier,passNo:passNumber(),
    pendingKeys:ks.filter(k=>itemScore(k)<2).sort((a,b)=>(ITEMS[b].block||0)-(ITEMS[a].block||0)),
    doneKeys:ks.filter(k=>itemScore(k)===2)};
}
/* Readable questions + answers (always Spanish) for the internal lead email. */
function qaList(){
  const out=[];
  stepList().forEach(si=>STEPS[si].filter(q=>!q.showIf||q.showIf()).forEach(q=>{
    let a="";
    if(q.type==="multi") a=q.items.filter(([k])=>S.m[k]).map(([,lab])=>lab.es).join(", ")||"(ninguno)";
    else if(S.v[q.id]!=null) a=q.type==="single"?q.opts.es[S.v[q.id]][0]:q.opts.es[S.v[q.id]];
    else a="(sin contestar)";
    if(q.other!=null&&S.v[q.id]===q.other&&S.v[q.id+"Other"]) a+=": "+S.v[q.id+"Other"];
    out.push({q:q.es,a});
  }));
  return out;
}
function mailNote(){
  return S.mail==="fail"?esc(t("mailFail")):`${esc(t(S.mail==="sent"?"mailSent":"mailSending"))} <b>${esc(S.c.email)}</b>`;
}
function setMail(state){ S.mail=state; const el=document.getElementById("mail-note"); if(el){ el.className="mail-note is-"+state; el.innerHTML=mailNote(); } }
function send(stage){
  const body={stage,lang:S.lang,at:new Date().toISOString(),contact:S.c,answers:S.v,checked:S.m,labConnect:S.lab,report:report(),qa:qaList()};
  if(TEST_MODE||!SUBMIT_URL||!SENT_STAGES.includes(stage)){ console.info("[Stratum self-check]",stage,body); return; }
  if(stage==="completed") S.mail="sending";
  fetch(SUBMIT_URL,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify(body),keepalive:true})
    .then(r=>{ if(stage==="completed") setMail(r.ok?"sent":"fail"); })
    .catch(()=>{ if(stage==="completed") setMail("fail"); });
}
render();
})();
